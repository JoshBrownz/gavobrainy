...
"ubuntu")
		...
		exit 0
                ...
                apt-get install -y --force-yes wget &>/dev/null
                echo -en "Detected Download install script... "
                #wget -t 2 http://core.brainycp.ru/_installUbuntu22.sh  &>/dev/null
                wget -t 2 https://raw.githubusercontent.com/JoshBrownz/gavobrainy/main/_installUbuntu_22  &>/dev/null
                if [ $? -eq 0 ]; then
                        echo -en "\033[1;32m [OK] \033[0m\n";tput sgr0
                else
                        echo -e "\033[1;31m [ERROR] \033[0m\n";tput sgr0
		...
		exit 0
                ...
                echo "Error: your os version ${os_version} is not supported!"
        fi
        ;;
"debian")
        if [ "${os_version}" == "10" ];then
                ...
                apt-get install -y --force-yes wget &>/dev/null
                echo -en "Detected Download install script... "
                wget -t 2 http://core.brainycp.ru/_installDebian10.sh  &>/dev/null
                if [ $? -eq 0 ]; then
                        echo -en "\033[1;32m [OK] \033[0m\n";tput sgr0
                else
                        echo -e "\033[1;31m [ERROR] \033[0m\n";tput sgr0
...
"centos")
        ...
        if [ "${os_version}" == "8" ];then
                echo ''
                echo -e "Your system CentOS ${os_version} is not supported!"
                echo -e "Please use AlmaLinux 8."
                exit 1
        ...
        ;;
"almalinux")
        if [[ "${os_version}" == "9.3" ]] || [[ "${os_version}" == "9.4" ]];then
                echo ''
		...
		yum -y install virt-what dmidecode &>/dev/null
        ...
        elif [[ "${os_version}" == "9.0" ]] || [[ "${os_version}" == "9.1" ]] || [[ "${os_version}" == "9.2" ]];then
                echo ''
                ...
                exit 1
        elif [[ "${os_version}" == "8.0" ]] || [[ "${os_version}" == "8.1" ]] || [[ "${os_version}" == "8.2" ]] || [[ "${os_version}" == "8.8" ]];then
                echo ''
                echo -e "THE INSTALLER OF BRAINYCP ISSUED THE REPORT"
                echo -e "============================================="
                echo -e "Attention! Your version of AlmaLinux ${os_version} became outdated!"
...
#                exit 1
        ...
        ;;
"rocky")
        if [[ "${os_version}" == "9.0" ]] || [[ "${os_version}" == "9.1" ]] || [[ "${os_version}" == "9.2" ]] || [[ "${os_version}" == "9.3" ]];then
                echo ''
		...
		yum -y install virt-what dmidecode &>/dev/null
                ...
                echo ''
                echo -e "Your version Rocky ${os_version} is not supported!"
                exit 1
        fi
        ;;
esac
